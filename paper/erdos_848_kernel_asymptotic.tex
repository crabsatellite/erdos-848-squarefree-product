...
\section{Introduction and Hall formulation}
...
\cite{Erdos848Original}.  We prove
...
\cite{Sawhney848}.  Sothanaphan made Sawhney's asymptotic inputs explicit and
obtained the threshold $N\geq2.64\cdot10^{17}$
\cite{Sothanaphan848}.  The argument below supplies an exact all-$N$ proof.
...
\toprule
Work & Proven range and conclusion & Why the problem was not yet closed &
New mechanism relevant here\\
\midrule
Sawhney (2025) &
The class $7\bmod25$ is extremal for every $N\geq N_0$, for some
unspecified $N_0$; a stability statement also identifies the two candidate
extremal classes. &
The argument is asymptotic: it neither gives $N_0$ nor treats the finite
prefix. &
...
$N\geq2.64\cdot10^{17}$. &
The interval $1\leq N<2.64\cdot10^{17}$ remains; a pointwise search over
that interval would not be a practical or transparent close. &
...
No range remains.  The four tail subranges are assembled into one theorem
for every $N\geq5\cdot10^6$. &
Hall reformulation, range compression, prefix colouring, and
valuation--cell--fibre descent to kernel-certified finite states.\\
\bottomrule
\end{tabularx}
\caption{Prior results and the all-$N$ close.}
\label{tab:prior-work}
\end{table}

The computational part of the proof is therefore not a pointwise
enumeration of every $N$.  The Hall reformulation replaces the extremal-set
question by a neighbourhood inequality; range compression reduces moving
endpoints to finitely many exact endpoint or cell conditions; and the
valuation--cell--fibre descent turns the unbounded tail into finite typed
states.  Only these compressed states are certified in Lean.  This use of
finite computation is in the tradition of explicit computational number
theory, where a bounded remainder is discharged by separately checkable
arithmetic certificates rather than treated as numerical evidence
\cite{HelfgottPlatt,OliveiraSilvaHerzogPardi}.

The squarefree-progression estimates behind the large-$N$ analysis belong to
the classical line of Prachar and Hooley
\cite{PracharSquarefreeAP,HooleySquarefreeAP}.  Our finite square-divisor
sieves are tailored to the Hall graph: they combine those distributional
inputs with exact residue, valuation, and fibre data rather than invoking an
asymptotic density statement as the final step.
...
All implications just invoked, including the exact row comparisons and their
QR semantics, are ordinary propositions in the accompanying Lean
formalization.  The paper-to-Lean map and trust boundary are stated in
Section~\ref{sec:machine-proof}.
\end{proof}
...
The formal artifact uses Lean~4 and Mathlib
\cite{Lean4,Mathlib}.  Its purpose is to check the exact finite arithmetic,
the paper-to-machine interfaces, and the terminal all-$N$ assembly with a
small trusted kernel.  This follows the established distinction between an
untrusted proof-producing computation and an independently replayed formal
certificate, as in large computational and formal proofs
...
\end{center}
The complete machine-readable theorem map contains twelve published Lean
declarations.  The load-bearing correspondence is summarized below.
...
\toprule
Paper component & Lean declarations\\
\midrule
Problem statement and sharp construction &
\texttt{OriginalProblem848Statement},
\texttt{originalA7\_has\_property}\\
\addlinespace
Exact prefix and the two low ranges through $5\cdot10^6$ &
\texttt{erdos848\_prefix\_close},
\texttt{erdos848\_first\_low\_close},
\texttt{erdos848\_second\_low\_close}\\
\addlinespace
The four tail intervals &
\texttt{PaperCertificateProvider.fiveToTenMillion},
\texttt{PaperCertificateProvider.tenToTwentyMillion},
\texttt{PaperCertificateProvider.twentyToFortyMillion},
\texttt{PaperCertificateProvider.fortyMillionTail}\\
\addlinespace
Unified tail theorem for every $N\geq5\cdot10^6$ &
\texttt{erdos848\_paper\_tail\_close}\\
\addlinespace
Terminal all-$N$ theorem &
\texttt{PaperGeneratedCertificateProvider.all\_N}\\
\bottomrule
\end{tabularx}
\caption{Load-bearing paper-to-Lean correspondence.}
\label{tab:lean-correspondence}
\end{table}

\subsection*{Certificate production and trust boundary}

The finite-state producers may use C++ or Python to find witnesses, compress
ranges, and emit Lean source.  They are not part of the trusted proof: Lean
does not rerun their search and no theorem is imported from an external
evaluator.  The generated data are presented to semantic checkers as ordinary
Lean terms; the checkers prove the required Hall, residue, valuation, cell,
and fibre statements; and the kernel validates the resulting proof terms.
Thus precomputed witnesses change proof-production time, not the logical
trust boundary.

The transitive publication cone contains exactly $30{,}572$ local Lean source
...
The repository pins the Lean toolchain and the Mathlib dependency graph.  The
machine-readable theorem map binds the paper labels to declarations, while
the proof contract and release manifest bind the TeX, PDF,
source closure, toolchain, terminal endpoint, and axiom policy.  From the
repository root, the release-state audit is
\begin{quote}\scriptsize\ttfamily
python -B scripts/check\_proof\_state.py
  --require-release-ready --audit-sources
\end{quote}
and the bounded builder and kernel gates are documented in the accompanying
\texttt{README.md}.  Optional precompiled \texttt{.olean} shards are a
hash-checked transport cache.  They can shorten replay, but they are neither
mathematical premises nor a substitute for kernel checking.
...
\bibitem{PracharSquarefreeAP}
K.~Prachar,
\emph{\"Uber die kleinste quadratfreie Zahl einer arithmetischen Reihe},
Monatsh. Math. 62 (1958), 173--176.
\url{https://resolver.sub.uni-goettingen.de/purl?362162050_0062|log15}

\bibitem{HooleySquarefreeAP}
C.~Hooley,
\emph{A note on square-free numbers in arithmetic progressions},
Bull. Lond. Math. Soc. 7 (1975), 133--138.
...
\bibitem{HelfgottPlatt}
H.~A. Helfgott and D.~J. Platt,
\emph{Numerical verification of the ternary Goldbach conjecture up to
$8.875\cdot10^{30}$},
Experiment. Math. 22 (2013), no.~4, 406--409.
\url{https://doi.org/10.1080/10586458.2013.831742}

\bibitem{OliveiraSilvaHerzogPardi}
...
\bibitem{GonthierOddOrder}
G.~Gonthier et al.,
\emph{A machine-checked proof of the Odd Order Theorem},
in Interactive Theorem Proving (ITP 2013), Lecture Notes in Comput. Sci.
7998, Springer, 2013, 163--179.
\url{https://doi.org/10.1007/978-3-642-39634-2_14}

\bibitem{HalesFlyspeck}
T.~C. Hales et al.,
\emph{A formal proof of the Kepler conjecture},
Forum Math. Pi 5 (2017), e2, 29 pp.
\url{https://doi.org/10.1017/fmp.2017.1}

\bibitem{Lean4}
L.~de Moura and S.~Ullrich,
\emph{The Lean 4 theorem prover and programming language},
in Automated Deduction -- CADE 28, Lecture Notes in Comput. Sci. 12699,
