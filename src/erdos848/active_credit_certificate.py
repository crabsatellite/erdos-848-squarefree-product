...
class ActiveCreditCertificate:
    ...
    worst_credit_deficit_allocation_count_upper_valid: bool
    worst_credit_deficit_allocation_reserve_prefix: bool
    worst_credit_deficit_allocation_prefix_reserve_valid: bool
    worst_credit_deficit_allocation_reserve_lower_bound_valid: bool
    ...
    worst_credit_deficit_allocation_seed_key_start: int
    worst_credit_deficit_allocation_seed_keys_consecutive: bool
    ...
    worst_credit_deficit_allocation_target_prefix_bound_from_seed_box_valid: bool
    ...
    observed_max_credit_deficit_allocation_count_upper_valid: bool
    observed_max_credit_deficit_allocation_reserve_prefix: bool
    observed_max_credit_deficit_allocation_prefix_reserve_valid: bool
    observed_max_credit_deficit_allocation_reserve_lower_bound_valid: bool
    ...
    observed_max_credit_deficit_allocation_seed_key_start: int
    observed_max_credit_deficit_allocation_seed_keys_consecutive: bool
    ...
    observed_max_credit_deficit_allocation_target_prefix_bound_from_seed_box_valid: bool
...
def active_credit_certificate(
    ...
    * strict-middle neighbors not already hit by O.
    ...
    worst_credit_deficit_allocation_count_upper_valid = True
    worst_credit_deficit_allocation_reserve_prefix = True
    worst_credit_deficit_allocation_prefix_reserve_valid = True
    worst_credit_deficit_allocation_reserve_lower_bound_valid = True
    ...
    worst_credit_deficit_allocation_seed_key_start = 0
    worst_credit_deficit_allocation_seed_keys_consecutive = True
    ...
    worst_credit_deficit_allocation_target_prefix_bound_from_seed_box_valid = True
    ...
    observed_max_credit_deficit_allocation_count_upper_valid = True
    observed_max_credit_deficit_allocation_reserve_prefix = True
    observed_max_credit_deficit_allocation_prefix_reserve_valid = True
    observed_max_credit_deficit_allocation_reserve_lower_bound_valid = True
    ...
    observed_max_credit_deficit_allocation_seed_key_start = 0
    observed_max_credit_deficit_allocation_seed_keys_consecutive = True
    ...
    observed_max_credit_deficit_allocation_target_prefix_bound_from_seed_box_valid = True
    ...
    def expand(
        ...
        nonlocal worst_credit_deficit_allocation_prefix_reserve_valid
        nonlocal worst_credit_deficit_allocation_reserve_lower_bound_valid
        ...
        nonlocal observed_max_credit_deficit_allocation_prefix_reserve_valid
        nonlocal observed_max_credit_deficit_allocation_reserve_lower_bound_valid
        ...
        if middle_size > 0:
            ...
            def allocation_count_upper_valid(deficit_length: int) -> bool:
                return middle_size <= new_middle_size + deficit_length

            def allocation_prefix_reserve_valid(deficit_length: int) -> bool:
                return all(
                    25 * i + base_residue in reserve_vertex_set
                    for i in range(deficit_length)
                )
            ...
            def allocation_target_prefix_bound_from_seed_box_valid(
                deficit_length: int,
                seed_keys: list[int],
            ) -> bool:
                if deficit_length == 0:
                    return True
                if len(seed_keys) < deficit_length:
                    return False
                last_seed = _deficit_seed_value(seed_keys[deficit_length - 1])
                return (
                    1 <= last_seed <= N
                    and 25 * deficit_length <= N + (25 - base_residue)
                )
            ...
            def allocation_indexed_source_witness_membership_valid(
                allocation_pairs: list[tuple[int, int]],
                witnesses: list[int],
            ) -> bool:
                if len(allocation_pairs) != len(witnesses):
                    return False
                chosen_values = {outside[index] for index in chosen}
                for prefix_index, (_pair, witness) in enumerate(
                    zip(allocation_pairs, witnesses)
                ):
                    prefix_pay = 25 * prefix_index + base_residue
                    if witness not in chosen_values:
                        return False
                    if not sf[prefix_pay * witness + 1]:
                        return False
            ...
            def allocation_seed_key_start(seed_keys: list[int]) -> int:
                return seed_keys[0] if seed_keys else 0

            def allocation_seed_keys_consecutive(seed_keys: list[int]) -> bool:
                start = allocation_seed_key_start(seed_keys)
                return all(seed_key == start + i for i, seed_key in enumerate(seed_keys))
            ...
            if credit_deficit > 0:
                ...
                if credit_deficit > observed_max_credit_deficit:
                    ...
                    observed_max_credit_deficit_allocation_count_upper_valid = (
                        allocation_count_upper_valid(credit_deficit)
                    )
                    ...
                    observed_max_credit_deficit_allocation_prefix_reserve_valid = (
                        allocation_prefix_reserve_valid(credit_deficit)
                    )
                    observed_max_credit_deficit_allocation_reserve_lower_bound_valid = (
                        allocation_reserve_lower_bound_valid(credit_deficit)
                    )
                    observed_seed_keys = [
                    ...
                    observed_max_credit_deficit_allocation_seed_key_start = (
                        allocation_seed_key_start(
                            observed_max_credit_deficit_allocation_seed_keys
                        )
                    )
                    observed_max_credit_deficit_allocation_seed_keys_consecutive = (
                        allocation_seed_keys_consecutive(
                            observed_max_credit_deficit_allocation_seed_keys
                        )
                    )
                    ...
                    observed_max_credit_deficit_allocation_target_prefix_bound_from_seed_box_valid = (
                        allocation_target_prefix_bound_from_seed_box_valid(
                            credit_deficit,
                            observed_max_credit_deficit_allocation_seed_keys,
                        )
                    )
            ...
            if defect < worst_credit_defect:
                ...
                worst_credit_deficit_allocation_count_upper_valid = (
                    allocation_count_upper_valid(credit_deficit)
                )
                worst_credit_deficit_allocation_reserve_prefix = (
                    [pay for _middle, pay in allocation_pairs] == base[:credit_deficit]
                )
                worst_credit_deficit_allocation_prefix_reserve_valid = (
                    allocation_prefix_reserve_valid(credit_deficit)
                )
                worst_credit_deficit_allocation_reserve_lower_bound_valid = (
                    allocation_reserve_lower_bound_valid(credit_deficit)
                )
                worst_seed_keys = [
                    _deficit_seed_key(middle) for middle, _pay in allocation_pairs
                ...
                worst_credit_deficit_allocation_seed_key_start = (
                    allocation_seed_key_start(worst_credit_deficit_allocation_seed_keys)
                )
                worst_credit_deficit_allocation_seed_keys_consecutive = (
                    allocation_seed_keys_consecutive(
                        worst_credit_deficit_allocation_seed_keys
                    )
                )
