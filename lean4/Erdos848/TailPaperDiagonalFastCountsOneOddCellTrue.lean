...
theorem TruncatedDiagonalAtomCounts.fastSum_oneOddCellTrue_eq_sum
    (counts : TruncatedDiagonalAtomCounts) (cell : Fin 9) :
    counts.fastSum (.oneOddCell true cell) =
      counts.sum (.oneOddCell true cell) := by
  apply counts.fastSum_eq_sum_of_code
  let g : TruncatedDiagonalAtomCode → Nat := fun code =>
    if (PaperDiagonalSelection.oneOddCell true cell).AcceptsAtom
        (truncatedDiagonalAtomEquiv.symm code)
      then counts.count (truncatedDiagonalAtomEquiv.symm code) else 0
  change counts.fastSum (.oneOddCell true cell) = ∑ code, g code
...
#print axioms TruncatedDiagonalAtomCounts.fastSum_oneOddCellTrue_eq_sum
