...
import Erdos848.TailTwentyMillionRootSurvivorSemantic
...
theorem twentyMillionRootSurvivorFinset_subset_coset
  ...
  have hbit := hparts.2
  rw [twentyMillionRootSupportWords_testBit] at hbit
  have hbaseBit := (Bool.and_eq_true_iff.mp hbit).2
  have hbase :=
    twentyMillionRootBaseWords_testBit_of_row_bound
...
theorem twentyMillionRootSurvivor_reciprocal_sum_le
    (normalLookup twistLookup : Nat → List Nat)
    (row : TwentyMillionRootRow)
    (squareCoset : Bool) (support : List Nat) :
  ...
  · intro m hm
    have hmem := twentyMillionRootSurvivorFinset_subset_coset
      normalLookup twistLookup row squareCoset support hm
    exact (Finset.mem_filter.mp hmem).2

theorem twentyMillionRootSurvivor_reciprocal_sum_le_ceiling
    {normalLookup twistLookup : Nat → List Nat}
    {supportLength : Nat}
    {support : List Nat}
    (hpass :
  ...
  exact (twentyMillionRootSurvivor_reciprocal_sum_le
...
#print axioms twentyMillionRootSurvivor_reciprocal_sum_le_ceiling
