...
import Erdos848.TailTwentyMillionRootActualSupport
...
lemma twentyMillionNormalHighPoint_parameter_mem
  ...
  obtain ⟨p, m, hpPrime, hpLarge, hequation, hmLower, hmUpper⟩ :=
    twentyMillionHighTailSquarePoint_has_bounded_quotient
  ...
  have hmSurvivor :
      m ∈ twentyMillionRootSurvivorFinset
        normalLookup twistLookup row squareCoset
        (fiveMillionPivotSupport pivot).dropLast := by
    apply mem_twentyMillionRootNormalSurvivorFinset_of_conditions
...
lemma twentyMillionFiveTwistHighPoint_parameter_mem
    {N : Nat} {B : Finset Nat} {pivot point baseResidue : Nat}
    {row : TwentyMillionRootRow} {squareCoset : Bool}
    {normalLookup twistLookup : Nat → List Nat}
    (hLower : 20_000_000 ≤ N)
    ...
    (hpUpper :
      ∀ q ∈ (fiveMillionPivotSupport pivot).dropLast,
        q ≤ twentyMillionRootNormalMaskBound) :
    point / 25 ∈
  ...
  have hmCoset : modFiveCosetAccepts squareCoset stored = true :=
    modFiveCosetAccepts_of_square_scaled
      hpFive hequationMod hcoset
  have hmSurvivor :
      stored ∈ twentyMillionRootSurvivorFinset
        normalLookup twistLookup row squareCoset
        (fiveMillionPivotSupport pivot).dropLast := by
    apply mem_twentyMillionRootTwistSurvivorFinset_of_conditions
...
#print axioms twentyMillionNormalHighPoint_parameter_mem
#print axioms twentyMillionFiveTwistHighPoint_parameter_mem
