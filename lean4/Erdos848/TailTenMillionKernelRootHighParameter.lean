...
import Erdos848.TailTenMillionKernelRootActualSupport
import Erdos848.TailTenMillionKernelRootReciprocal

namespace Erdos848

/-!
# Actual high-tail points in the twenty-million survivor sets

This is the literal map from an arbitrary high-tail point to a transformed
parameter.  The untrusted tables only provide `hmaskPass`; all divisibility,
congruence, mod-five, valuation, and range facts are proved here.
-/

set_option maxHeartbeats 0
set_option maxRecDepth 1000000

lemma tenMillionNormalHighPoint_parameter_mem
    ...
    point / 25 ∈
      transformedParameterSet (N / 25 + 2) (N / row.split) (25 * pivot)
        (pivot * baseResidue + 1)
        (tenMillionKernelRootSurvivorFinset
          normalLookup twistLookup row squareCoset
  ...
  obtain ⟨p, m, hpPrime, hpLarge, hequation, hmLower, hmUpper⟩ :=
    tenMillionHighTailSquarePoint_has_bounded_quotient
      row hpivotUpper hpointHigh
  have hYLarge : 5 < N / row.split := by
    cases row <;>
  ...
  have hmSurvivor :
      m ∈ tenMillionKernelRootSurvivorFinset
        normalLookup twistLookup row squareCoset
        (fiveMillionPivotSupport pivot).dropLast := by
    apply mem_tenMillionKernelRootNormalSurvivorFinset_of_conditions
      hrow hmLower hmUpper (hquotient htransformed) hmCoset hmaskPass
    intro q hq
...
lemma tenMillionFiveTwistHighPoint_parameter_mem
    {N : Nat} {B : Finset Nat} {pivot point baseResidue : Nat}
    {row : TenMillionKernelRootRow} {squareCoset : Bool}
    {normalLookup twistLookup : Nat → List Nat}
    (hLower : 10_000_000 ≤ N)
    ...
    (hpUpper :
      ∀ q ∈ (fiveMillionPivotSupport pivot).dropLast,
        q ≤ tenMillionKernelRootNormalMaskBound) :
    point / 25 ∈
      transformedParameterSet (N / 25 + 2) (N / row.split) (5 * pivot)
        ((pivot * baseResidue + 1) / 5)
        (tenMillionKernelRootSurvivorFinset
          normalLookup twistLookup row squareCoset
          (fiveMillionPivotSupport pivot).dropLast) := by
  ...
  have hmFive : 5 ∣ raw := by
    ...
    · exact hm
  let stored := raw / 5
  have hmEq : 5 * stored = raw := Nat.mul_div_cancel' hmFive
  have hmStoredLower : 1 ≤ stored := by
    have hmFiveLe : 5 ≤ raw := Nat.le_of_dvd (by omega) hmFive
    exact (Nat.le_div_iff_mul_le (by norm_num : 0 < 5)).mpr (by omega)
  have hmStoredUpper : stored ≤ row.bound :=
  ...
  have hmStoredFive : ¬ 5 ∣ stored := by
    ...
    have hleft : 25 ∣ 5 * stored := by
      ...
      omega
    rw [hmEq] at hleft
    have hpSquareNotFive : ¬ 5 ∣ p ^ 2 := by
      intro h
      exact hpFive ((show Nat.Prime 5 by norm_num).dvd_of_dvd_pow h)
  ...
  have hmCoset : modFiveCosetAccepts squareCoset stored = true :=
    modFiveCosetAccepts_of_square_scaled
      hpFive hequationMod hcoset
  have hmSurvivor :
      stored ∈ tenMillionKernelRootSurvivorFinset
        normalLookup twistLookup row squareCoset
        (fiveMillionPivotSupport pivot).dropLast := by
    apply mem_tenMillionKernelRootTwistSurvivorFinset_of_conditions
      hrow hmStoredLower hmStoredUpper (hquotient htransformed)
      hmCoset hmaskPass hpUpper
    intro q hq
    have hqSupport : q ∈ fiveMillionPivotSupport pivot :=
      List.mem_of_mem_dropLast hq
    have hsquare :=
...
#print axioms tenMillionNormalHighPoint_parameter_mem
#print axioms tenMillionFiveTwistHighPoint_parameter_mem
