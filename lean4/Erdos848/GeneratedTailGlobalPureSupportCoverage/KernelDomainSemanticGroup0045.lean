...
theorem kernelSupportPrimeGroup0045_subset
    {p : ℕ}
    (hpGroup : p ∈ kernelSupportPrimeGroup0045) :
    p ∈ kernelSupportPrimes := by
  simp only [kernelSupportPrimes, List.mem_append]
  exact Or.inl (Or.inl (Or.inr (hpGroup)))

theorem kernelSupportPrimeSemanticGroup0045
    {p : ℕ}
    (hLower : 26228 ≤ p) (hUpper : p ≤ 26861)
    (hp : Erdos848.FiveMillionSupportPrime p) :
    p ∈ kernelSupportPrimes := by
  apply kernelSupportPrimeGroup0045_subset
  interval_cases p <;>
