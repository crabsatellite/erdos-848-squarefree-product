...
theorem kernelSupportPrimeGroup0046_subset
    {p : ℕ}
    (hpGroup : p ∈ kernelSupportPrimeGroup0046) :
    p ∈ kernelSupportPrimes := by
  simp only [kernelSupportPrimes, List.mem_append]
  exact Or.inl (Or.inr (hpGroup))

theorem kernelSupportPrimeSemanticGroup0046
    {p : ℕ}
    (hLower : 26862 ≤ p) (hUpper : p ≤ 27551)
    (hp : Erdos848.FiveMillionSupportPrime p) :
    p ∈ kernelSupportPrimes := by
  apply kernelSupportPrimeGroup0046_subset
  interval_cases p <;>
