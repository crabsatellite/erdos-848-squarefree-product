...
/-!
# Completeness of the fixed forty-million support domain

An actual increasing support of length at most seven and product below
`40_000_000` contributes its `dropLast` prefix to the generated root table.
...
private theorem twentyMillion_dropLast_sublist_self {alpha : Type*}
...
theorem actualTwentyMillionSupport_dropLast_mem_rootFeasible
    ...
    (hFinite : support.prod < 40_000_000) :
    support.dropLast ∈ twentyMillionRootFeasiblePrefixes k := by
  by_cases hkZero : k = 0
  · have hSupportLength : support.length = 0 := hLength.trans hkZero
    have hSupport : support = [] := by simpa using hSupportLength
    simp [hSupport, twentyMillionRootFeasiblePrefixes, hkZero]
  ...
  have hPrefixPairwise : supportPrefix.Pairwise (· < ·) :=
    hIncreasing.sublist (twentyMillion_dropLast_sublist_self support)
  ...
  have hEnumerated := feasibleSupportPrefixes_complete
    (bound := 40_000_000) (need := k - 1) (product := 1)
    hSublist
  ...
  have hTake : selected.take (k - 1) = supportPrefix := by
    simp [selected, hPrefixLength]
  simp only [twentyMillionRootFeasiblePrefixes, dif_neg hkZero]
  simpa [hTake] using hEnumerated

#print axioms actualTwentyMillionSupport_dropLast_mem_rootFeasible
