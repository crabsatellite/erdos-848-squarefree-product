...
theorem TruncatedDiagonalAtomCounts.fastSum_oneOddCellFalse_eq_sum
    (counts : TruncatedDiagonalAtomCounts) (cell : Fin 9) :
    counts.fastSum (.oneOddCell false cell) =
      counts.sum (.oneOddCell false cell) := by
  apply counts.fastSum_eq_sum_of_code
  let g : TruncatedDiagonalAtomCode → Nat := fun code =>
    if (PaperDiagonalSelection.oneOddCell false cell).AcceptsAtom
        (truncatedDiagonalAtomEquiv.symm code)
      then counts.count (truncatedDiagonalAtomEquiv.symm code) else 0
  change counts.fastSum (.oneOddCell false cell) = ∑ code, g code
...
#print axioms TruncatedDiagonalAtomCounts.fastSum_oneOddCellFalse_eq_sum
