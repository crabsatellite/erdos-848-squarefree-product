...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      successCriterion := "A theorem of the shape `SquarefreeAPHallCertificate` is proved without ``."
    },
    {
