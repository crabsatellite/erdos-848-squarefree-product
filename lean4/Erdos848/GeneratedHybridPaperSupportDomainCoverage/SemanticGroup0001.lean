...
theorem supportPrimeGroup0001_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0001) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inl hpGroup)

theorem supportPrimeSemanticGroup0001
    {p : ℕ}
  ...
  apply supportPrimeGroup0001_subset
  interval_cases p <;>
