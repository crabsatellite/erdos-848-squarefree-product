...
theorem supportPrimeGroup0014_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0014) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup))))))))))))))

theorem supportPrimeSemanticGroup0014
    {p : ℕ}
  ...
  apply supportPrimeGroup0014_subset
  interval_cases p <;>
