...
theorem supportPrimeGroup0010_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0010) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup))))))))))

theorem supportPrimeSemanticGroup0010
    {p : ℕ}
  ...
  apply supportPrimeGroup0010_subset
  interval_cases p <;>
