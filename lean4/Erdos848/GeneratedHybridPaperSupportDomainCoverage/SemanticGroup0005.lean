...
theorem supportPrimeGroup0005_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0005) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup)))))

theorem supportPrimeSemanticGroup0005
    {p : ℕ}
  ...
  apply supportPrimeGroup0005_subset
  interval_cases p <;>
