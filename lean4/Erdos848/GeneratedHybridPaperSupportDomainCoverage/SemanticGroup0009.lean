...
theorem supportPrimeGroup0009_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0009) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup)))))))))

theorem supportPrimeSemanticGroup0009
    {p : ℕ}
  ...
  apply supportPrimeGroup0009_subset
  interval_cases p <;>
