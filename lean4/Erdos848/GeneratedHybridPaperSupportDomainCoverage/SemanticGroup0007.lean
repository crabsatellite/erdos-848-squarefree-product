...
theorem supportPrimeGroup0007_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0007) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup)))))))

theorem supportPrimeSemanticGroup0007
    {p : ℕ}
  ...
  apply supportPrimeGroup0007_subset
  interval_cases p <;>
