...
theorem supportPrimeGroup0013_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0013) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup)))))))))))))

theorem supportPrimeSemanticGroup0013
    {p : ℕ}
  ...
  apply supportPrimeGroup0013_subset
  interval_cases p <;>
