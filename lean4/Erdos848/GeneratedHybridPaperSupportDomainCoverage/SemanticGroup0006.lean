...
theorem supportPrimeGroup0006_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0006) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup))))))

theorem supportPrimeSemanticGroup0006
    {p : ℕ}
    (hLower : 2664 ≤ p) (hUpper : p ≤ 3181)
  ...
  apply supportPrimeGroup0006_subset
  interval_cases p <;>
