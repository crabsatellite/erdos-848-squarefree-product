...
theorem supportPrimeGroup0011_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0011) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup)))))))))))

theorem supportPrimeSemanticGroup0011
    {p : ℕ}
  ...
  apply supportPrimeGroup0011_subset
  interval_cases p <;>
