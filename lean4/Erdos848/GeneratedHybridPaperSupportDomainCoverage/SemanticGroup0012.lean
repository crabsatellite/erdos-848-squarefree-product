...
theorem supportPrimeGroup0012_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0012) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup))))))))))))

theorem supportPrimeSemanticGroup0012
    {p : ℕ}
  ...
  apply supportPrimeGroup0012_subset
  interval_cases p <;>
