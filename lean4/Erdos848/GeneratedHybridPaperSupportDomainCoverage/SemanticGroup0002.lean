...
theorem supportPrimeGroup0002_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0002) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inl hpGroup))

theorem supportPrimeSemanticGroup0002
    {p : ℕ}
  ...
  apply supportPrimeGroup0002_subset
  interval_cases p <;>
