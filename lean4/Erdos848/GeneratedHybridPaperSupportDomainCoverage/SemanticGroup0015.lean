...
theorem supportPrimeGroup0015_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0015) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup)))))))))))))))

theorem supportPrimeSemanticGroup0015
    {p : ℕ}
  ...
  apply supportPrimeGroup0015_subset
  interval_cases p <;>
