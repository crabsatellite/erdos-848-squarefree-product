...
theorem supportPrimeGroup0004_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0004) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup))))

theorem supportPrimeSemanticGroup0004
    {p : ℕ}
    (hLower : 1628 ≤ p) (hUpper : p ≤ 2137)
  ...
  apply supportPrimeGroup0004_subset
  interval_cases p <;>
