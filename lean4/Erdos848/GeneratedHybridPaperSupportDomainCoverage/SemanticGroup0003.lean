...
theorem supportPrimeGroup0003_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0003) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inl hpGroup)))

theorem supportPrimeSemanticGroup0003
    {p : ℕ}
  ...
  apply supportPrimeGroup0003_subset
  interval_cases p <;>
