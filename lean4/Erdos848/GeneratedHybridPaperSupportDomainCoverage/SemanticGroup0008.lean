...
theorem supportPrimeGroup0008_subset
    {p : ℕ} (hpGroup : p ∈ supportPrimeGroup0008) :
    p ∈ supportPrimes := by
  simp only [supportPrimes, List.mem_append]
  exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl hpGroup))))))))

theorem supportPrimeSemanticGroup0008
    {p : ℕ}
  ...
  apply supportPrimeGroup0008_subset
  interval_cases p <;>
