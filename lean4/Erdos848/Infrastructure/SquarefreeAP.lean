...
/-- Interpret a finite code list as a total source-index selector. -/
def OppositeFiniteOffsetListSelector
    (codes : List OppositeFiniteOffsetCode) (k : Nat) :
    OppositeFiniteOffsetCode :=
  match codes.get? k with
  | some code => code
...
/-- Coherence between the typed finite-offset value and source-index target. -/
def OppositeFiniteOffsetSourceIndexTargetCoherent
    (k : Nat) (code : OppositeFiniteOffsetCode) : Prop :=
  OppositeFiniteOffsetCodeValue (EighteenSourceFromIndex k) code =
    OppositeFiniteOffsetSourceIndexTargetValue k code

/--
Source-index target validity with underflow excluded by direct target-value
coherence.
-/
def OppositeFiniteOffsetSourceIndexTargetCoherentBoxValid
    (N k : Nat) (code : OppositeFiniteOffsetCode) : Prop :=
...
theorem oppositeFiniteOffsetSourceIndexCodeNonUnderflow_of_targetCoherent
    ...
    (h : OppositeFiniteOffsetSourceIndexTargetCoherent k code) :
    OppositeFiniteOffsetSourceIndexCodeNonUnderflow k code := by
  cases code
  · simp [OppositeFiniteOffsetSourceIndexTargetCoherent,
      ...
      OppositeFiniteOffsetSourceIndexCodeNonUnderflow] at h ⊢
    omega
  · simp [OppositeFiniteOffsetSourceIndexTargetCoherent,
      OppositeFiniteOffsetCodeValue, OppositeFiniteOffsetCode.toNat,
      OppositeFiniteOffsetValue, EighteenSourceFromIndex,
  ...
  · simp [OppositeFiniteOffsetSourceIndexTargetCoherent,
      ...
      OppositeFiniteOffsetSourceIndexCodeNonUnderflow] at h ⊢
    omega
  · simp [OppositeFiniteOffsetSourceIndexCodeNonUnderflow]
  · simp [OppositeFiniteOffsetSourceIndexCodeNonUnderflow]
  · simp [OppositeFiniteOffsetSourceIndexCodeNonUnderflow]
  · simp [OppositeFiniteOffsetSourceIndexCodeNonUnderflow]

/-- Coherent target-value boxedness supplies target-box validity. -/
theorem oppositeFiniteOffsetSourceIndexTargetBoxValid_of_targetCoherentBoxValid
    {N k : Nat} {code : OppositeFiniteOffsetCode}
    (h : OppositeFiniteOffsetSourceIndexTargetCoherentBoxValid N k code) :
    OppositeFiniteOffsetSourceIndexTargetBoxValid N k code :=
  And.intro
...
/-- Box-free local target-validity for every index in the finite selector list. -/
def OppositeFiniteOffsetListSelectorLengthTargetValid
    (N : Nat) (codes : List OppositeFiniteOffsetCode) : Prop :=
  forall k : Nat, k < codes.length ->
    OppositeFiniteOffsetSourceIndexTargetValid N k
      (OppositeFiniteOffsetListSelector codes k)
...
/-- Target-value coherence and boxedness for every finite selector-list index. -/
def OppositeFiniteOffsetListSelectorLengthTargetCoherentBoxValid
    (N : Nat) (codes : List OppositeFiniteOffsetCode) : Prop :=
  forall k : Nat, k < codes.length ->
...
/--
Local no-collision condition for the bandwidth-three source-index target map.
Any global collision must occur inside this six-index window.
-/
def OppositeFiniteOffsetListSelectorLengthLocalShiftInjective
    (_N : Nat) (codes : List OppositeFiniteOffsetCode) : Prop :=
  forall k1 k2 : Nat,
    k1 < codes.length ->
    k2 < codes.length ->
...
certificate.
-/
def OppositeFiniteOffsetListSelectorLengthBoxFreeValidMatching
    (N : Nat) (codes : List OppositeFiniteOffsetCode) : Prop :=
  codes.length = OppositeFiniteOffsetSourceCount N /\
...
/--
Length-exact valid matching with coherent direct target-value boxedness on
every list index.
-/
def OppositeFiniteOffsetListSelectorLengthTargetCoherentBoxValidMatching
    (N : Nat) (codes : List OppositeFiniteOffsetCode) : Prop :=
  codes.length = OppositeFiniteOffsetSourceCount N /\
...
/--
Length-exact valid matching with target coherence and local six-window
injectivity.
-/
def OppositeFiniteOffsetListSelectorLengthTargetCoherentBoxLocalValidMatching
    (N : Nat) (codes : List OppositeFiniteOffsetCode) : Prop :=
  codes.length = OppositeFiniteOffsetSourceCount N /\
  OppositeFiniteOffsetListSelectorLengthTargetCoherentBoxValid N codes /\
...
theorem oppositeFiniteOffsetSourceIndexShiftTarget_collision_gap_le_six
    ...
    (h :
      OppositeFiniteOffsetSourceIndexShiftTarget k1 c1 =
        OppositeFiniteOffsetSourceIndexShiftTarget k2 c2)
    (hlt : k1 < k2) :
    k2 <= k1 + 6 := by
  cases c1 <;> cases c2 <;>
    simp [OppositeFiniteOffsetSourceIndexShiftTarget] at h <;> omega

/-- Local six-window no-collision supplies global source-index shift injectivity. -/
theorem oppositeFiniteOffsetListSelectorLengthShiftInjective_of_localShiftInjective
    {N : Nat} {codes : List OppositeFiniteOffsetCode}
    (hLocal :
      OppositeFiniteOffsetListSelectorLengthLocalShiftInjective N codes) :
    OppositeFiniteOffsetListSelectorLengthShiftInjective N codes := by
  intro k1 k2 hk1 hk2 hShift
  by_cases hlt : k1 < k2
  · have hgap :
        k2 <= k1 + 6 :=
      oppositeFiniteOffsetSourceIndexShiftTarget_collision_gap_le_six
        hShift hlt
    exact False.elim ((hLocal k1 k2 hk1 hk2 hlt hgap) hShift)
  · by_cases hgt : k2 < k1
    · have hgap :
          k1 <= k2 + 6 :=
        oppositeFiniteOffsetSourceIndexShiftTarget_collision_gap_le_six
...
/--
Box-free length-local validity supplies the previous length-local package with
explicit boxed-source premises.
-/
theorem oppositeFiniteOffsetListSelectorLengthValidMatching_of_lengthBoxFreeValidMatching
    {N : Nat} {codes : List OppositeFiniteOffsetCode}
    (h : OppositeFiniteOffsetListSelectorLengthBoxFreeValidMatching N codes) :
    OppositeFiniteOffsetListSelectorLengthValidMatching N codes :=
  And.intro h.left
    (And.intro
      (by
        intro k hk _hBox
        exact h.right.left k hk)
      (by
...
/--
Coherent target-value boxedness supplies the previous direct target-box
validity package.
-/
theorem oppositeFiniteOffsetListSelectorLengthTargetBoxValidMatching_of_targetCoherentBoxValidMatching
    {N : Nat} {codes : List OppositeFiniteOffsetCode}
    (h :
      OppositeFiniteOffsetListSelectorLengthTargetCoherentBoxValidMatching
        N codes) :
  ...
  And.intro h.left
    (And.intro
      (by
        intro k hk
        exact
          oppositeFiniteOffsetSourceIndexTargetBoxValid_of_targetCoherentBoxValid
            (h.right.left k hk))
      h.right.right)

/--
Target coherence plus local six-window injectivity supplies the previous
globally injective target-coherence package.
-/
theorem oppositeFiniteOffsetListSelectorLengthTargetCoherentBoxValidMatching_of_localValidMatching
    {N : Nat} {codes : List OppositeFiniteOffsetCode}
    (h :
...
/--
Generated prefix-pair allocation whose seed keys are certified by prefix
injectivity instead of list-level nodup.
-/
def ActiveStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
  ...
  Exists fun oppositeWitnessSourceIndex : Nat -> Nat =>
    deficitLength =
      @familySize N (StrictMiddlePart 7 B) decMid -
        @familySize N (IncrementalStrictMiddleNeighbor N 7 B) decNewMid /\
    (forall i : Nat, i < deficitLength ->
      forall j : Nat, j < deficitLength ->
        deficitSeedPrefixKey i = deficitSeedPrefixKey j -> i = j) /\
    (forall i : Nat, i < deficitLength ->
      StrictMiddlePart 7 B
        (ActiveStrictMiddleCreditDeficitSeedValue
          (deficitSeedPrefixKey i))) /\
...
/--
Generated prefix-pair allocation whose deficit length is certified by additive
count balance instead of truncated subtraction.
-/
def ActiveStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
...
/--
Generated prefix-pair allocation whose seed keys are certified by strict
monotonicity on the deficit prefix.
-/
def ActiveStrictMiddleCreditDeficitSeedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
  ...
  Exists fun oppositeWitnessSourceIndex : Nat -> Nat =>
    ...
    (forall i : Nat, i < deficitLength ->
      forall j : Nat, j < deficitLength ->
        i < j -> deficitSeedPrefixKey i < deficitSeedPrefixKey j) /\
    (forall i : Nat, i < deficitLength ->
      StrictMiddlePart 7 B
        (ActiveStrictMiddleCreditDeficitSeedValue
          (deficitSeedPrefixKey i))) /\
...
/--
Generated prefix-pair allocation whose seed keys are one consecutive prefix
`start + i`.
-/
def ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
    ...
    (decNewMid :
      DecidablePred (IncrementalStrictMiddleNeighbor N 7 B)) : Prop :=
  Exists fun deficitLength : Nat =>
  Exists fun deficitSeedStartKey : Nat =>
  Exists fun oppositeWitnessSourceIndex : Nat -> Nat =>
    @familySize N (StrictMiddlePart 7 B) decMid =
      @familySize N (IncrementalStrictMiddleNeighbor N 7 B) decNewMid +
        deficitLength /\
...
/--
Consecutive seed-key allocation without an explicit target-prefix box bound.
The global projection derives that scalar bound from boundedness of the last
seed value.
-/
def ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
...
/--
Consecutive seed-key allocation whose indexed witness field only carries source
membership; the opposite-carrier proof is derived from `EighteenSourceFromIndex`.
-/
def ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
    ...
    (forall i : Nat, i < deficitLength ->
      StrictMiddlePart 7 B
        (ActiveStrictMiddleCreditDeficitSeedValue
          (deficitSeedStartKey + i))) /\
    (forall i : Nat, i < deficitLength ->
      B (EighteenSourceFromIndex (oppositeWitnessSourceIndex i)) /\
        ForbiddenSquarefreeEdge (25 * i + 7)
          (EighteenSourceFromIndex (oppositeWitnessSourceIndex i))) /\
    (forall b : Nat,
      OppositeOutsidePart 7 B b ->
...
/--
Consecutive seed-key allocation whose source witnesses are only asserted
existentially prefix-by-prefix; Lean chooses the witness function.
-/
def ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
    ...
    (forall i : Nat, i < deficitLength ->
      StrictMiddlePart 7 B
        (ActiveStrictMiddleCreditDeficitSeedValue
          (deficitSeedStartKey + i))) /\
    (forall i : Nat, i < deficitLength ->
      Exists fun sourceIndex : Nat =>
        B (EighteenSourceFromIndex sourceIndex) /\
          ForbiddenSquarefreeEdge (25 * i + 7)
            (EighteenSourceFromIndex sourceIndex)) /\
    (forall b : Nat,
      OppositeOutsidePart 7 B b ->
      deficitLength <=
        OppositeFiniteOffsetSourceIndexShiftTarget
...
/--
Consecutive seed-key allocation with only the count upper bound needed for the
active-credit deficit inequality.
-/
def ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
...
/--
Seed-key-free deficit allocation: only the count upper bound, target-prefix
box bound, prefix source witnesses, and mate lower bound remain live.
-/
def ActiveStrictMiddleCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (_decReserve :
...
/-- Seed-key-free deficit allocation reduced to a scalar reserve lower bound. -/
def ActiveStrictMiddleCreditDeficitCountUpperReserveLowerBoundAllocation
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (decReserve :
...
/-- Seed-key-free deficit allocation reduced to a pure reserve-dominance inequality. -/
def ActiveStrictMiddleCreditDeficitReserveDominance
    (N : Nat) (B : Nat -> Prop)
    (decMid : DecidablePred (StrictMiddlePart 7 B))
    (offsetIndex : Nat -> OppositeFiniteOffsetCode)
    (decReserve :
...
/--
Source-index active credit allocation whose seed keys are certified by prefix
injectivity instead of list-level nodup.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation whose deficit length is certified by
additive count balance.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation whose seed keys are certified by strict
prefix monotonicity.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation whose seed keys are one consecutive
prefix `start + i`.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
bound.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation whose indexed witness field carries only
source membership.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation whose source witnesses are only
existentially asserted prefix-by-prefix.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation with count upper bound and existential
source witnesses.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation with total generated prefix-pair source
witnesses.
-/
def 
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/-- Source-index active credit allocation with no seed-key carrier field. -/
def GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation with total target-prefix bounds and
existential source witnesses.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/-- Source-index active credit allocation as pure reserve dominance. -/
def GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitReserveDominance
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation as pure reserve dominance for every
bounded outside clique.  The empty strict-middle case is discharged in Lean.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalReserveDominance
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index active credit allocation as a total scalar reserve lower-bound
certificate.  The witness `deficitLength` is explicit for every bounded outside
clique.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperReserveLowerBoundAllocation
    (N : Nat) (offsetIndex : Nat -> OppositeFiniteOffsetCode) : Prop :=
  forall (B : Nat -> Prop)
      (decMid : DecidablePred (StrictMiddlePart 7 B))
      (decReserve : DecidablePred
...
/--
Source-index split certificate whose seed keys are certified by prefix
injectivity instead of list-level nodup.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
     N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/--
Source-index split certificate whose deficit length is certified by additive
count balance.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
     N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/--
Source-index split certificate whose seed keys are certified by strict prefix
monotonicity.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
     N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/--
Source-index split certificate whose seed keys are one consecutive prefix
`start + i`.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
...
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
     N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/--
Source-index split certificate whose indexed witness field carries only source
membership.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
     N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/--
Source-index split certificate whose active-credit side carries only
prefix-wise existential source witnesses.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
     N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/--
Source-index split certificate whose active-credit side carries only the count
upper bound and prefix-wise existential source witnesses.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
     N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/-- Source-index split certificate with no seed-key carrier field. -/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
...
/-- Source-index split certificate as a scalar reserve lower bound. -/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCountUpperReserveLowerBoundAllocationCertificate :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
    GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCountUpperReserveLowerBoundAllocation N
      (OppositeFiniteOffsetTemplateWindowRepairCode windows)

/-- Source-index split certificate as pure reserve dominance. -/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitReserveDominanceCertificate :
    Prop :=
  forall N : Nat, Exists fun windows : List OppositeFiniteOffsetRepairWindow =>
    GlobalOppositeFiniteOffsetEighteenTypedSourceIndexTemplateWindowRepairValidMatching
      N windows /\
...
/-- Source-index split certificate with a finite list selector and reserve dominance. -/
...
/--
Source-index split certificate with a finite length-exact list and box-free
local matching clauses.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthBoxFreeCreditDeficitReserveDominanceCertificate :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthBoxFreeValidMatching N codes /\
    GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitReserveDominance N
...
/--
Source-index split certificate with target-value coherence and boxedness in the
finite length-exact list certificate.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthTargetCoherentBoxCreditDeficitReserveDominanceCertificate :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthTargetCoherentBoxValidMatching N codes /\
    GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitReserveDominance N
      (OppositeFiniteOffsetListSelector codes)

/--
Source-index split certificate with target coherence and local six-window
injectivity in the finite length-exact list certificate.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthTargetCoherentBoxLocalCreditDeficitReserveDominanceCertificate :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthTargetCoherentBoxLocalValidMatching N codes /\
    GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitReserveDominance N
      (OppositeFiniteOffsetListSelector codes)
...
/--
Source-index split certificate with count-additive generated prefix-pair source
witnesses in the finite gap-indexed list certificate.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthTargetCoherentBoundaryBoxGapValidMatching N codes /\
     N
...
/--
Source-index split certificate with total reserve dominance; the strict-middle
empty branch is not part of the live oracle.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapCreditTotalDeficitReserveDominanceCertificate :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthTargetCoherentBoundaryBoxGapValidMatching N codes /\
...
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapCreditTotalDeficitCountUpperReserveLowerBoundAllocationCertificate :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthTargetCoherentBoundaryBoxGapValidMatching N codes /\
...
/--
Source-index split certificate with total target-prefix source witnesses for
the gap-indexed finite list certificate.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapCreditTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthTargetCoherentBoundaryBoxGapValidMatching N codes /\
    GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation N
      (OppositeFiniteOffsetListSelector codes)

/--
Source-index split certificate with total generated prefix-pair source witnesses
for the gap-indexed finite list certificate.
-/
def  :
    Prop :=
  forall N : Nat, Exists fun codes : List OppositeFiniteOffsetCode =>
    OppositeFiniteOffsetListSelectorLengthTargetCoherentBoundaryBoxGapValidMatching N codes /\
     N
      (OppositeFiniteOffsetListSelector codes)
...
/--
Current source-index split certificate whose seed keys are certified by prefix
injectivity instead of list-level nodup.
-/
def  :
    Prop :=
  

/--
Current source-index split certificate whose deficit length is certified by
additive count balance.
-/
def  :
    Prop :=
  

/--
Current source-index split certificate whose seed keys are certified by strict
prefix monotonicity.
-/
def  :
    Prop :=
  

/--
Current source-index split certificate whose seed keys are one consecutive
prefix `start + i`.
-/
def  :
    Prop :=
...
/--
Current source-index split certificate without an explicit target-prefix box
bound.
-/
def  :
    Prop :=
  

/--
Current source-index split certificate whose indexed witness field carries only
source membership.
-/
def  :
    Prop :=
  

/-- Current source-index split certificate with existential source witnesses. -/
def  :
    Prop :=
  

/-- Current source-index split certificate with count upper bound. -/
def  :
    Prop :=
  

/-- Current source-index split certificate with no seed-key carrier field. -/
...
/-- Current source-index split certificate as a scalar reserve lower bound. -/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCountUpperReserveLowerBoundAllocationCertificate :
    Prop :=
  GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCountUpperReserveLowerBoundAllocationCertificate

/-- Current source-index split certificate as pure reserve dominance. -/
...
/-- Current source-index split certificate as finite list-selector reserve dominance. -/
...
/--
Current source-index split certificate as a length-exact, box-free finite
...
/--
Current source-index split certificate as a target-coherent, target-boxed,
length-exact finite list-selector reserve-dominance certificate.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoxDeficitReserveDominanceCertificate :
    Prop :=
  GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthTargetCoherentBoxCreditDeficitReserveDominanceCertificate

/--
Current source-index split certificate as a local-window target-coherent,
target-boxed, length-exact finite list-selector reserve-dominance certificate.
-/
...
/--
Current source-index split certificate with count-additive generated prefix-pair
source witnesses.
-/
def  :
    Prop :=
  

/--
Current source-index split certificate with gap-indexed local injectivity and
total reserve dominance.
-/
...
/--
Current source-index split certificate with total target-prefix source
witnesses.
-/
def GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate :
    Prop :=
  GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapCreditTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate

/--
Current source-index split certificate with total generated prefix-pair source
witnesses.
-/
def  :
    Prop :=
  
...
/--
Additive-count allocation supplies the previous truncated-difference length
field.
-/
theorem activeStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_seedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    ActiveStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  rcases hCountAdditive with
    ⟨deficitLength, deficitSeedPrefixKey, oppositeWitnessSourceIndex,
      hCountAdditive, hKeyInjectiveOnPrefix, hSeedStrictMiddle, hPrefixBound,
  ...
  exact
    ⟨deficitLength, deficitSeedPrefixKey, oppositeWitnessSourceIndex, hLength,
      hKeyInjectiveOnPrefix, hSeedStrictMiddle, hPrefixBound,
      hIndexedOppositeWitness, hMateSourceIndexLower⟩

/--
Strictly increasing seed keys supply the previous prefix-injectivity field.
-/
theorem activeStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_seedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    ActiveStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  rcases hStrictMono with
    ⟨deficitLength, deficitSeedPrefixKey, oppositeWitnessSourceIndex,
      hCountAdditive, hKeyStrictMono, hSeedStrictMiddle, hPrefixBound,
      hIndexedOppositeWitness, hMateSourceIndexLower⟩
  have hKeyInjectiveOnPrefix :
      forall i : Nat, i < deficitLength ->
      forall j : Nat, j < deficitLength ->
        deficitSeedPrefixKey i = deficitSeedPrefixKey j -> i = j := by
    intro i hi j hj hEq
    by_cases hSame : i = j
    · exact hSame
    · have hOrder : i < j \/ j < i := by omega
      cases hOrder with
      | inl hij =>
          have hKeyLt := hKeyStrictMono i hi j hj hij
          omega
      | inr hji =>
          have hKeyLt := hKeyStrictMono j hj i hi hji
          omega
  exact
    ⟨deficitLength, deficitSeedPrefixKey, oppositeWitnessSourceIndex,
      hCountAdditive, hKeyInjectiveOnPrefix, hSeedStrictMiddle, hPrefixBound,
...
/--
Boxedness of the last generated seed value implies the generated target-prefix
upper bound.
-/
theorem activeStrictMiddleCreditDeficitSeedValue_targetPrefixBound_of_lastSeedBox
    {N seedStart deficitLength : Nat}
    (hpos : 0 < deficitLength)
    (hBox :
      InBox N
        (ActiveStrictMiddleCreditDeficitSeedValue
          (seedStart + (deficitLength - 1)))) :
    25 * deficitLength <= N + 18 := by
  have hUpper :
      ActiveStrictMiddleCreditDeficitSeedValue
          (seedStart + (deficitLength - 1)) <= N := hBox.right
  unfold ActiveStrictMiddleCreditDeficitSeedValue at hUpper
  by_cases h0 : seedStart + (deficitLength - 1) = 0
  · simp [h0] at hUpper
    omega
  · by_cases h1 : seedStart + (deficitLength - 1) = 1
    · simp [h0, h1] at hUpper
      omega
    · simp [h0, h1] at hUpper
      have hkey : 2 <= seedStart + (deficitLength - 1) := by omega
      omega

/--
The target-prefix box bound is derived from boundedness of the last consecutive
seed value.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hNoPrefix :
    ...
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  rcases hNoPrefix B decMid decReserve decNewMid hB hClique hMid with
    ⟨deficitLength, deficitSeedStartKey, oppositeWitnessSourceIndex,
      hCountAdditive, hSeedStrictMiddle, hIndexedOppositeWitness,
      hMateSourceIndexLower⟩
  have hPrefixBound : 25 * deficitLength <= N + 18 := by
    by_cases hzero : deficitLength = 0
    · omega
    · have hpos : 0 < deficitLength := Nat.pos_of_ne_zero hzero
      have hi : deficitLength - 1 < deficitLength := by omega
      have hSeedLast :=
        hSeedStrictMiddle (deficitLength - 1) hi
      have hSeedBox :
          InBox N
            (ActiveStrictMiddleCreditDeficitSeedValue
              (deficitSeedStartKey + (deficitLength - 1))) :=
        (hB _ hSeedLast.left).left
      exact
        activeStrictMiddleCreditDeficitSeedValue_targetPrefixBound_of_lastSeedBox
          hpos hSeedBox
  exact
    ⟨deficitLength, deficitSeedStartKey, oppositeWitnessSourceIndex,
      hCountAdditive, hSeedStrictMiddle, hPrefixBound, hIndexedOppositeWitness,
      hMateSourceIndexLower⟩

/--
Existential source witnesses supply a source-witness function by choice.
-/
theorem activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    {decNewMid :
      DecidablePred (IncrementalStrictMiddleNeighbor N 7 B)}
    (hExistsSourceWitness :
      ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        N B decMid offsetIndex decReserve decNewMid) :
    ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  classical
  rcases hExistsSourceWitness with
    ⟨deficitLength, deficitSeedStartKey, hCountAdditive,
      hSeedStrictMiddle, hExistsSourceWitness, hMateSourceIndexLower⟩
  let oppositeWitnessSourceIndex : Nat -> Nat := fun i =>
    if hi : i < deficitLength then
      Classical.choose (hExistsSourceWitness i hi)
    else
      0
  have hIndexedSourceWitness :
      forall i : Nat, i < deficitLength ->
      B (EighteenSourceFromIndex (oppositeWitnessSourceIndex i)) /\
        ForbiddenSquarefreeEdge (25 * i + 7)
          (EighteenSourceFromIndex (oppositeWitnessSourceIndex i)) := by
    intro i hi
    have hChoose := Classical.choose_spec (hExistsSourceWitness i hi)
    simpa [oppositeWitnessSourceIndex, hi] using hChoose
  exact
    ⟨deficitLength, deficitSeedStartKey, oppositeWitnessSourceIndex,
      hCountAdditive, hSeedStrictMiddle, hIndexedSourceWitness,
      hMateSourceIndexLower⟩

/--
Source-membership indexed witnesses supply the previous indexed opposite-witness
field; the opposite carrier is automatic for `25*k+18`.
-/
theorem activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  rcases hSourceWitness with
    ⟨deficitLength, deficitSeedStartKey, oppositeWitnessSourceIndex,
      hCountAdditive, hSeedStrictMiddle, hIndexedSourceWitness,
      hMateSourceIndexLower⟩
  have hIndexedOppositeWitness :
      forall i : Nat, i < deficitLength ->
      OppositeOutsidePart 7 B (EighteenSourceFromIndex
        (oppositeWitnessSourceIndex i)) /\
        ForbiddenSquarefreeEdge (25 * i + 7)
...
/--
Consecutive seed keys supply the previous strict-monotone seed-key field.
-/
theorem activeStrictMiddleCreditDeficitSeedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    ActiveStrictMiddleCreditDeficitSeedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  rcases hConsecutive with
    ⟨deficitLength, deficitSeedStartKey, oppositeWitnessSourceIndex,
      hCountAdditive, hSeedStrictMiddle, hPrefixBound, hIndexedOppositeWitness,
      hMateSourceIndexLower⟩
  refine
    ⟨deficitLength, (fun i => deficitSeedStartKey + i),
      oppositeWitnessSourceIndex, hCountAdditive, ?_, ?_, hPrefixBound,
      hIndexedOppositeWitness, hMateSourceIndexLower⟩
  · intro i hi j hj hij
    exact Nat.add_lt_add_left hij deficitSeedStartKey
  · intro i hi
    exact hSeedStrictMiddle i hi

/--
Seed-key prefix-injective allocation supplies the previous seed-key nodup
field.
-/
theorem activeStrictMiddleCreditDeficitSeedKeyNodupCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_seedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    ActiveStrictMiddleCreditDeficitSeedKeyNodupCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  rcases hKeyInjective with
    ⟨deficitLength, deficitSeedPrefixKey, oppositeWitnessSourceIndex, hLength,
      hKeyInjectiveOnPrefix, hSeedStrictMiddle, hPrefixBound,
      hIndexedOppositeWitness, hMateSourceIndexLower⟩
  have hKeyNodup :
      ((List.range deficitLength).map deficitSeedPrefixKey).Nodup := by
    have hRangeNodup : (List.range deficitLength).Nodup :=
      List.nodup_range deficitLength
    apply list_nodup_map_of_inj_on hRangeNodup
    intro i j hi hj hEq
    exact hKeyInjectiveOnPrefix i
      (by simpa [List.mem_range] using hi) j
      (by simpa [List.mem_range] using hj) hEq
  exact
    ⟨deficitLength, deficitSeedPrefixKey, oppositeWitnessSourceIndex, hLength,
...
/-- If there is no strict-middle source, reserve dominance is the zero-count case. -/
theorem activeStrictMiddleCreditDeficitReserveDominance_of_no_strictMiddle
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
...
/-- If there is no strict-middle source, the scalar lower-bound allocation uses zero. -/
theorem activeStrictMiddleCreditDeficitCountUpperReserveLowerBoundAllocation_of_no_strictMiddle
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
...
/--
If there is no strict-middle source, the target-prefix/source-witness
allocation is empty.
-/
theorem activeStrictMiddleCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_no_strictMiddle
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
...
/--
Count-additive generated prefix-pair source witnesses supply the weaker
count-upper generated prefix-pair source-witness allocation.
-/
theorem activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    ActiveStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  rcases h with
    ⟨deficitLength, deficitSeedStartKey, hCountAdditiveUpper,
      hSeedStrictMiddle, hExistsSourceWitness, hMateSourceIndexLower⟩
  refine
    ⟨deficitLength, deficitSeedStartKey, ?_, hSeedStrictMiddle,
      hExistsSourceWitness, hMateSourceIndexLower⟩
  omega

/--
If there is no strict-middle source, the generated-prefix source-witness
allocation is empty.
-/
theorem activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_no_strictMiddle
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
...
/--
Generated prefix-pair source witnesses supply the seed-key-free target-prefix
source-witness allocation.
-/
theorem activeStrictMiddleCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
    ...
    ActiveStrictMiddleCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
      N B decMid offsetIndex decReserve decNewMid := by
  rcases h with
    ⟨deficitLength, deficitSeedStartKey, hCountUpper, hSeedStrictMiddle,
      hExistsSourceWitness, hMateSourceIndexLower⟩
  have hPrefixBound : 25 * deficitLength <= N + 18 := by
    by_cases hzero : deficitLength = 0
    · omega
    · have hpos : 0 < deficitLength := Nat.pos_of_ne_zero hzero
      have hi : deficitLength - 1 < deficitLength := by omega
      have hSeedLast := hSeedStrictMiddle (deficitLength - 1) hi
      have hSeedBox :
          InBox N (ActiveStrictMiddleCreditDeficitSeedValue
            (deficitSeedStartKey + (deficitLength - 1))) :=
        (hB _ hSeedLast.left).left
      exact activeStrictMiddleCreditDeficitSeedValue_targetPrefixBound_of_lastSeedBox
        hpos hSeedBox
  exact
    ⟨deficitLength, hCountUpper, hPrefixBound, hExistsSourceWitness,
      hMateSourceIndexLower⟩

/--
Target-prefix bounds and source witnesses directly certify each prefix reserve
point.
-/
theorem activeStrictMiddleCreditDeficitCountUpperPrefixReserveAllocation_of_countUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
  ...
  rcases h with
    ⟨deficitLength, hCountUpper, hPrefixBound, hExistsSourceWitness,
      hMateSourceIndexLower⟩
  refine ⟨deficitLength, hCountUpper, ?_⟩
  intro i hi
  rcases hExistsSourceWitness i hi with ⟨sourceIndex, hbSource, hEdge⟩
  constructor
  · unfold SquarefreeNeighborInCandidate
    refine ⟨?_, ?_, ?_⟩
    · unfold InBox
      constructor <;> omega
    · unfold CandidateCarrier
    ...
    · exact
        ⟨EighteenSourceFromIndex sourceIndex,
          ⟨⟨hbSource,
              oppositeCandidateCarrier_seven_of_candidate_eighteen
                (candidateCarrier_eighteenSourceFromIndex sourceIndex)⟩,
            hEdge⟩⟩
  · intro hImage
    rcases hImage with ⟨b, hbOpp, hMateEq⟩
    have hLower := hMateSourceIndexLower b hbOpp
    have hEq :
        25 *
            OppositeFiniteOffsetSourceIndexShiftTarget
              (CandidateClassIndex b) (offsetIndex (CandidateClassIndex b)) +
        ...
        25 * i + 7 := by
      simpa [OppositeFiniteOffsetSourceIndexMate,
        OppositeFiniteOffsetSourceIndexTargetValue] using hMateEq
    omega
...
theorem activeStrictMiddleCreditDeficitReserveDominance_of_countUpperReserveLowerBoundAllocation
    ...
    ActiveStrictMiddleCreditDeficitReserveDominance
      N B decMid offsetIndex decReserve decNewMid := by
  rcases h with ⟨deficitLength, hCountUpper, hReserveLower⟩
  unfold ActiveStrictMiddleCreditDeficitReserveDominance
  omega

/--
The positive strict-middle reserve obligation extends to the total reserve
obligation because Lean closes the empty strict-middle branch by count zero.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalReserveDominance_of_reserveDominance
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
  ...
  · apply activeStrictMiddleCreditDeficitReserveDominance_of_no_strictMiddle
    intro b hb
    exact hMid (Exists.intro b hb)

/-- Total reserve dominance restricts back to the positive strict-middle form. -/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitReserveDominance_of_totalReserveDominance
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalReserveDominance
        N offsetIndex) :
  ...
  exact h B decMid decReserve decNewMid hB hClique

/--
The positive scalar lower-bound allocation extends to the total scalar
lower-bound allocation by giving the empty strict-middle branch deficit zero.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperReserveLowerBoundAllocation_of_countUpperReserveLowerBoundAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
...
/--
The positive target-prefix/source-witness allocation extends to the total form
by giving the empty strict-middle branch a zero target prefix.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_countUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
        N offsetIndex) :
    GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique
  by_cases hMid : Exists fun b : Nat => StrictMiddlePart 7 B b
  · exact h B decMid decReserve decNewMid hB hClique hMid
  · apply activeStrictMiddleCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_no_strictMiddle
    intro b hb
    exact hMid (Exists.intro b hb)

/--
The positive generated-prefix source-witness allocation extends to the total
form by giving the empty strict-middle branch a zero generated prefix.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
      
        N offsetIndex) :
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique
  by_cases hMid : Exists fun b : Nat => StrictMiddlePart 7 B b
  · exact h B decMid decReserve decNewMid hB hClique hMid
  · apply activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_no_strictMiddle
    intro b hb
    exact hMid (Exists.intro b hb)

/--
The positive count-additive generated-prefix source-witness allocation extends
to the total count-upper generated-prefix form by closing the empty
strict-middle branch in Lean.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
  ...
  by_cases hMid : Exists fun b : Nat => StrictMiddlePart 7 B b
  · exact
      activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        (h B decMid decReserve decNewMid hB hClique hMid)
  · apply activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_no_strictMiddle
    intro b hb
    exact hMid (Exists.intro b hb)

/--
Total generated-prefix source-witness allocation supplies the total
target-prefix/source-witness allocation.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_totalConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
      
        N offsetIndex) :
  ...
  intro B decMid decReserve decNewMid hB hClique
  exact
    activeStrictMiddleCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hB (h B decMid decReserve decNewMid hB hClique)

/--
Total target-prefix/source-witness allocation supplies the total prefix reserve
witness allocation.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperPrefixReserveAllocation_of_totalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
        N offsetIndex) :
  ...
  intro B decMid decReserve decNewMid hB hClique
  exact
    activeStrictMiddleCreditDeficitCountUpperPrefixReserveAllocation_of_countUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
      (h B decMid decReserve decNewMid hB hClique)

/-- Total scalar reserve lower-bound allocation implies total reserve dominance. -/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalReserveDominance_of_totalCountUpperReserveLowerBoundAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperReserveLowerBoundAllocation
        N offsetIndex) :
  ...
  intro B decMid decReserve decNewMid hB hClique
  exact
    activeStrictMiddleCreditDeficitReserveDominance_of_countUpperReserveLowerBoundAllocation
      (h B decMid decReserve decNewMid hB hClique)

/-- Total reserve dominance canonically supplies the scalar reserve lower-bound allocation. -/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperReserveLowerBoundAllocation_of_totalReserveDominance
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalReserveDominance
        N offsetIndex) :
...
field.
-/
theorem _of_seedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hCountAdditive :
    ...
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  exact activeStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_seedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (hCountAdditive B decMid decReserve decNewMid hB hClique hMid)

/--
Window-repair additive-count certificate supplies the previous
truncated-difference length certificate.
-/
theorem _of_seedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  rcases h N with ⟨windows, hValid, hCountAdditive⟩
  exact ⟨windows, hValid,
    _of_seedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hCountAdditive⟩

/--
Current additive-count certificate supplies the previous truncated-difference
length certificate.
-/
theorem _of_seedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
  ...
  simpa [
    ...
    ]
    using
      _of_seedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h

/--
Strictly increasing seed keys supply the previous prefix-injectivity
certificate.
-/
theorem _of_seedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hStrictMono :
    ...
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  exact activeStrictMiddleCreditDeficitSeedKeyInjectiveCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_seedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (hStrictMono B decMid decReserve decNewMid hB hClique hMid)

/--
Window-repair strict-monotone seed-key certificate supplies the previous
prefix-injectivity certificate.
-/
theorem _of_seedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  rcases h N with ⟨windows, hValid, hStrictMono⟩
  exact ⟨windows, hValid,
    _of_seedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hStrictMono⟩

/--
Current strict-monotone seed-key certificate supplies the previous
prefix-injectivity certificate.
-/
theorem _of_seedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
  ...
  simpa [
    ...
    ]
    using
      _of_seedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h

/--
Existential source witnesses supply the previous source-witness certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hExistsSourceWitness :
      
        N offsetIndex) :
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  exact activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (hExistsSourceWitness B decMid decReserve decNewMid hB hClique hMid)

/--
Window-repair existential source-witness certificate supplies the previous
source-witness certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  rcases h N with ⟨windows, hValid, hExistsSourceWitness⟩
  exact ⟨windows, hValid,
    _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hExistsSourceWitness⟩

/--
Current existential source-witness certificate supplies the previous
source-witness certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
    ...
     := by
  simpa [
    ,
    ...
    using
      _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h

/--
Source-membership indexed witnesses supply the previous indexed
opposite-witness certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hSourceWitness :
      
        N offsetIndex) :
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  exact activeStrictMiddleCreditDeficitConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (hSourceWitness B decMid decReserve decNewMid hB hClique hMid)

/--
Window-repair source-witness certificate supplies the previous indexed
opposite-witness certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  rcases h N with ⟨windows, hValid, hSourceWitness⟩
  exact ⟨windows, hValid,
    _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hSourceWitness⟩

/--
Current source-witness certificate supplies the previous indexed
opposite-witness certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  simpa [
    ,
    ...
    using
      _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h

/--
Window-repair no-prefix certificate supplies the target-prefix bound from the
last consecutive seed value.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  rcases h N with ⟨windows, hValid, hNoPrefix⟩
  exact ⟨windows, hValid,
    _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hNoPrefix⟩

/--
Current no-prefix certificate supplies the target-prefix bound from the last
consecutive seed value.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
  ...
  simpa [
    ...
    ]
    using
      _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h

/--
Consecutive seed keys supply the previous strict-monotone seed-key certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hConsecutive :
      
        N offsetIndex) :
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  exact activeStrictMiddleCreditDeficitSeedKeyStrictMonoCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (hConsecutive B decMid decReserve decNewMid hB hClique hMid)

/--
Window-repair consecutive seed-key certificate supplies the previous
strict-monotone seed-key certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  rcases h N with ⟨windows, hValid, hConsecutive⟩
  exact ⟨windows, hValid,
    _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hConsecutive⟩

/--
Current consecutive seed-key certificate supplies the previous strict-monotone
seed-key certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  simpa [
    ...
    ]
    using
      _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h
...
field.
-/
theorem _of_seedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hKeyInjective :
      
        N offsetIndex) :
    
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  exact activeStrictMiddleCreditDeficitSeedKeyNodupCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation_of_seedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (hKeyInjective B decMid decReserve decNewMid hB hClique hMid)

/--
Window-repair seed-key prefix-injective certificate supplies the previous
seed-key nodup certificate.
-/
theorem _of_seedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  rcases h N with ⟨windows, hValid, hKeyInjective⟩
  exact ⟨windows, hValid,
    _of_seedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hKeyInjective⟩

/--
Current seed-key prefix-injective certificate supplies the previous seed-key
nodup certificate.
-/
theorem _of_seedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
  ...
  simpa [
    ...
    ]
    using
      _of_seedKeyInjectiveCarrierFreeCountFreeUpperFreeSeedStrictMiddleTargetPrefixBoxBoundIndexedOppositeWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h
...
/--
Count-upper prefix reserve targets directly supply source-index deficit
capacity, without the old pair-list exact-deficit equality chain.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCapacity_of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hCountUpper :
      
        N offsetIndex) :
    GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCapacity
      N offsetIndex := by
  intro B decMid decReserve decNewMid hB hClique hMid
  rcases hCountUpper B decMid decReserve decNewMid hB hClique hMid with
    ⟨deficitLength, deficitSeedStartKey, hCountUpper, hSeedStrictMiddle,
      hExistsSourceWitness, hMateSourceIndexLower⟩
  have hPrefixBound : 25 * deficitLength <= N + 18 := by
    by_cases hzero : deficitLength = 0
...
/-- Window-repair count-upper certificate directly supplies deficit capacity. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCapacity_of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCapacityCertificate := by
  intro N
  rcases h N with ⟨windows, hValid, hCountUpper⟩
  exact ⟨windows, hValid,
    globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCapacity_of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      hCountUpper⟩

/-- Current count-upper certificate directly supplies deficit capacity. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCapacity_of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
  ...
  simpa [
    ...
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCapacityCertificate]
    using
      globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCapacity_of_consecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h

/--
Seed-key-free count-upper prefix reserve targets directly supply source-index
deficit capacity.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCapacity_of_countUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hAlloc :
      GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
        N offsetIndex) :
...
/-- Window-repair seed-key-free certificate directly supplies deficit capacity. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCapacity_of_countUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCapacityCertificate := by
  intro N
  ...
  exact ⟨windows, hValid,
    globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCapacity_of_countUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
      hAlloc⟩

/-- Current seed-key-free certificate directly supplies deficit capacity. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCapacity_of_countUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    (h :
...
/-- Reserve dominance supplies the scalar reserve-lower-bound allocation. -/
theorem activeStrictMiddleCreditDeficitCountUpperReserveLowerBoundAllocation_of_reserveDominance
    {N : Nat} {B : Nat -> Prop}
    {decMid : DecidablePred (StrictMiddlePart 7 B)}
    {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    {decReserve :
...
/-- Source-index reserve dominance supplies the scalar reserve-lower-bound allocation. -/
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCountUpperReserveLowerBoundAllocation_of_reserveDominance
    {N : Nat} {offsetIndex : Nat -> OppositeFiniteOffsetCode}
    (hAlloc :
      GlobalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitReserveDominance
        N offsetIndex) :
...
/-- Window-repair reserve dominance supplies the scalar reserve-lower-bound certificate. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCountUpperReserveLowerBoundAllocation_of_reserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCountUpperReserveLowerBoundAllocationCertificate := by
  intro N
  rcases h N with ⟨windows, hValid, hDominance⟩
  exact ⟨windows, hValid,
...
/-- Current reserve dominance supplies the scalar reserve-lower-bound certificate. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCountUpperReserveLowerBoundAllocation_of_reserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCountUpperReserveLowerBoundAllocationCertificate := by
  simpa [
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitReserveDominanceCertificate,
...
/-- Finite list-selector reserve dominance supplies the direct selector certificate. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditSelectorDeficitReserveDominance_of_listSelector
    (h :
...
/--
Box-free length-exact finite list-selector reserve dominance supplies the
previous length-exact list-selector certificate.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthDeficitReserveDominance_of_lengthBoxFreeListSelector
    (h :
    ...
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthDeficitReserveDominanceCertificate := by
  intro N
  cases h N with
  | intro codes hPack =>
    exact Exists.intro codes
...
/--
Target-coherent length-exact finite list-selector reserve dominance supplies
the previous target-boxed list-selector certificate.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetBoxDeficitReserveDominance_of_lengthTargetCoherentBoxListSelector
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoxDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetBoxDeficitReserveDominanceCertificate := by
  intro N
  cases h N with
  | intro codes hPack =>
    exact Exists.intro codes
...
/--
Local-window target-coherent list-selector reserve dominance supplies the
previous globally injective target-coherent list-selector certificate.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoxDeficitReserveDominance_of_lengthTargetCoherentBoxLocalListSelector
    (h :
    ...
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoxDeficitReserveDominanceCertificate := by
  intro N
  cases h N with
  | intro codes hPack =>
    exact Exists.intro codes
...
/--
Positive strict-middle reserve dominance supplies the total reserve-dominance
version of the gap-indexed list-selector certificate.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitReserveDominance_of_lengthTargetCoherentBoundaryBoxGapReserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapDeficitReserveDominanceCertificate) :
...
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapDeficitReserveDominance_of_totalReserveDominance
    (h :
    ...
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapDeficitReserveDominanceCertificate := by
  intro N
  cases h N with
  | intro codes hPack =>
    exact Exists.intro codes
      (And.intro hPack.left
        (globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitReserveDominance_of_totalReserveDominance
          hPack.right))

/--
Total scalar lower-bound list-selector data supplies total reserve dominance.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitReserveDominance_of_totalCountUpperReserveLowerBoundAllocation
    (h :
  ...
  | intro codes hPack =>
    exact Exists.intro codes
      (And.intro hPack.left
        (globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalReserveDominance_of_totalCountUpperReserveLowerBoundAllocation
          hPack.right))

/--
Total reserve dominance canonically supplies the scalar lower-bound
list-selector certificate.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperReserveLowerBoundAllocation_of_totalReserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitReserveDominanceCertificate) :
...
/--
Total target-prefix/source-witness list-selector data supplies the total prefix
reserve witness certificate.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperPrefixReserveAllocation_of_totalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperPrefixReserveAllocationCertificate := by
  intro N
  cases h N with
  | intro codes hPack =>
    exact Exists.intro codes
      (And.intro hPack.left
        (globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperPrefixReserveAllocation_of_totalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
          hPack.right))

/--
Total generated-prefix source-witness list-selector data supplies the total
target-prefix/source-witness certificate.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_totalConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate := by
  intro N
  cases h N with
  | intro codes hPack =>
    exact Exists.intro codes
      (And.intro hPack.left
        (globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_totalConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
          hPack.right))

/--
Count-additive generated-prefix source-witness list-selector data supplies the
total count-upper generated-prefix source-witness certificate.
-/
theorem _of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
     := by
  intro N
  cases h N with
  | intro codes hPack =>
    exact Exists.intro codes
      (And.intro hPack.left
        (_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
          hPack.right))
...
theorem globalFiniteOffsetMiddleCompressionEighteenSourceIndexMateActiveCreditDeficitCapacity_of_countUpperReserveLowerBoundAllocation
  ...
  intro B decMid decReserve decNewMid hB hClique hMid
  rcases hAlloc B decMid decReserve decNewMid hB hClique hMid with
    ⟨deficitLength, hCountUpper, hReserveLower⟩
  unfold ActiveStrictMiddleCreditDeficitCapacity
  omega

/-- Window-repair reserve-lower-bound certificate supplies deficit capacity. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCapacity_of_countUpperReserveLowerBoundAllocation
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCountUpperReserveLowerBoundAllocationCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexTemplateWindowRepairCreditDeficitCapacityCertificate := by
  intro N
...
/-- Current reserve-lower-bound certificate supplies deficit capacity. -/
theorem globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCapacity_of_countUpperReserveLowerBoundAllocation
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCountUpperReserveLowerBoundAllocationCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditDeficitCapacityCertificate := by
  simpa [
...
/-- Finite list-selector reserve dominance supplies decoded squarefree-boxed compression. -/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorReserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
...
/--
Box-free length-exact finite list-selector reserve dominance supplies decoded
squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthBoxFreeReserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthBoxFreeDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
...
decoded squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoxReserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoxDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
    globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetBoxReserveDominance
      (globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetBoxDeficitReserveDominance_of_lengthTargetCoherentBoxListSelector
        h)

/--
Local-window target-coherent finite list-selector reserve dominance supplies
decoded squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoxLocalReserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoxLocalDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
...
/--
Total reserve-dominance gap-indexed finite list-selector data supplies decoded
squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapTotalReserveDominance
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitReserveDominanceCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
    globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapReserveDominance
      (globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapDeficitReserveDominance_of_totalReserveDominance
        h)

/--
Total scalar reserve lower-bound gap-indexed finite list-selector data supplies
decoded squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapTotalCountUpperReserveLowerBoundAllocation
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperReserveLowerBoundAllocationCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
...
/--
Total target-prefix source-witness gap-indexed finite list-selector data
supplies decoded squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
    (h :
      GlobalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocationCertificate) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
    globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapTotalCountUpperPrefixReserveAllocation
      (globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperPrefixReserveAllocation_of_totalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
        h)

/--
Total generated-prefix source-witness gap-indexed finite list-selector data
supplies decoded squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapTotalConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
    globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapTotalCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation
      (globalFiniteOffsetMiddleCompressionEighteenTypedMateSplitSourceIndexShiftCreditListSelectorLengthTargetCoherentBoundaryBoxGapTotalDeficitCountUpperTargetPrefixBoundExistsSourceWitnessMateSourceIndexLowerBoundAllocation_of_totalConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h)

/--
Count-additive generated-prefix source-witness gap-indexed finite list-selector
data supplies decoded squarefree-boxed compression.
-/
theorem globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
    (h :
      ) :
    GlobalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCertificate := by
  exact
    globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapTotalConsecutiveSeedKeyCarrierFreeCountUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
      (_of_consecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
        h)
...
the simple typed mate, stated with count-additive generated prefix-pair source
witnesses that imply count-upper/target-prefix source witnesses and the prefix
reserve witness, and with the empty strict-middle reserve branch closed in
Lean.
-/
...
theorem finiteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxedCut :
  ...
  globalFiniteOffsetMiddleCompressionEighteenDecodedSquarefreeBoxed_of_sourceIndexListSelectorLengthTargetCoherentBoundaryBoxGapConsecutiveSeedKeyCarrierFreeCountAdditiveUpperFreeSeedStrictMiddleExistsSourceWitnessMateSourceIndexLowerBoundLengthGeneratedPrefixPairSeedValuePrefixEdgeShiftTargetNoImagePairListAllocation
