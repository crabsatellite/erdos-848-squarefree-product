...
theorem TruncatedDiagonalAtomCounts.fastSum_concentrated2_eq_sum
    (counts : TruncatedDiagonalAtomCounts) (cell : Fin 9) :
    counts.fastSum (.concentrated ⟨2, by decide⟩ cell) =
      counts.sum (.concentrated ⟨2, by decide⟩ cell) := by
  apply counts.fastSum_eq_sum_of_code
  let g : TruncatedDiagonalAtomCode → Nat := fun code =>
    if (PaperDiagonalSelection.concentrated
        ⟨2, by decide⟩ cell).AcceptsAtom
        (truncatedDiagonalAtomEquiv.symm code)
      then counts.count (truncatedDiagonalAtomEquiv.symm code) else 0
  change counts.fastSum (.concentrated ⟨2, by decide⟩ cell) =
    ∑ code, g code
  ...
  fin_cases cell <;>
    simp [fastSum, PaperDiagonalSelection.AcceptsAtom,
      TruncatedDiagonalAtom.modFourClass,
      TruncatedDiagonalAtom.modNineCell, sum9, sum49, evenOneTotal,
      TruncatedDiagonalAtomCounts.count, g, truncatedDiagonalAtomEquiv] <;>
    omega

#print axioms TruncatedDiagonalAtomCounts.fastSum_concentrated2_eq_sum
