...
import Erdos848.TailTenMillionKernelRootSupportComplete

namespace Erdos848

/-!
# Actual support and quotient bounds for the ten-million root rows

The generated table only covers support lengths at most six and quotient
...
`10,000,000 ≤ N < 20,000,000`.
-/

set_option maxHeartbeats 0
set_option maxRecDepth 1000000

theorem mixedSupport_length_le_six_of_prod_lt_twentyMillion
    (support : List Nat)
    (hPrimes : ∀ p ∈ support, FiveMillionSupportPrime p)
    (hIncreasing : support.Pairwise (· < ·))
    (hProduct : support.prod < 20_000_000) :
    support.length ≤ 6 := by
  by_contra hLength
  have hSeven : 7 ≤ support.length := by omega
  obtain ⟨p₀, p₁, p₂, p₃, p₄, p₅, p₆, rest, rfl⟩ :
      ∃ p₀ p₁ p₂ p₃ p₄ p₅ p₆ rest,
        support = p₀ :: p₁ :: p₂ :: p₃ :: p₄ :: p₅ :: p₆ :: rest := by
    match support with
    | p₀ :: p₁ :: p₂ :: p₃ :: p₄ :: p₅ :: p₆ :: rest =>
        exact ⟨p₀, p₁, p₂, p₃, p₄, p₅, p₆, rest, rfl⟩
    | [] | [_] | [_, _] | [_, _, _] | [_, _, _, _] |
  ...
  have hp₆Lower : 23 ≤ p₆ := by
    ...
    interval_cases p₆ <;>
      norm_num [FiveMillionSupportPrime] at hp₆ <;> omega
  have hPrefix :
      22_309_287 ≤ p₀ * p₁ * p₂ * p₃ * p₄ * p₅ * p₆ := by
    calc
      22_309_287 = 3 * 7 * 11 * 13 * 17 * 19 * 23 := by norm_num
      _ ≤ p₀ * p₁ * p₂ * p₃ * p₄ * p₅ * p₆ := by gcongr
  have hRestPositive : 0 < rest.prod := List.prod_pos fun p hp =>
    (hPrimes p (by simp [hp])).1.pos
  have hFull :
      22_309_287 ≤
        p₀ * p₁ * p₂ * p₃ * p₄ * p₅ * p₆ * rest.prod := by
    calc
      22_309_287 ≤ p₀ * p₁ * p₂ * p₃ * p₄ * p₅ * p₆ := hPrefix
      _ = p₀ * p₁ * p₂ * p₃ * p₄ * p₅ * p₆ * 1 := by simp
      _ ≤ p₀ * p₁ * p₂ * p₃ * p₄ * p₅ * p₆ * rest.prod := by
        exact Nat.mul_le_mul_left _
          (show 1 ≤ rest.prod from hRestPositive)
  have hProduct' :
      p₀ * p₁ * p₂ * p₃ * p₄ * p₅ * p₆ * rest.prod <
        20_000_000 := by
    simpa only [List.prod_cons, Nat.mul_assoc] using hProduct
  omega

theorem actualTenMillionPivotSupport_length_le_six
    {N : Nat} {B : Finset Nat} {pivot : Nat}
    (hUpper : N < 20_000_000)
    (hBout : Erdos848OutsideSet N B)
    (hpivotResidual : pivot ∈ hallResidual N B) :
    (fiveMillionPivotSupport pivot).length ≤ 6 := by
  ...
  apply mixedSupport_length_le_six_of_prod_lt_twentyMillion
...
theorem actualTenMillionSupport_dropLast_lt_normalMaskBound
    {k : Nat} (hk : k ≤ 6) (support : List Nat)
    (hLength : support.length = k)
    (hPrimes : ∀ p ∈ support, FiveMillionSupportPrime p)
    (hIncreasing : support.Pairwise (· < ·))
    (hProduct : support.prod < 20_000_000) :
    ∀ p ∈ support.dropLast, p < tenMillionKernelRootNormalMaskBound := by
  intro p hp
  ...
  simpa [tenMillionKernelRootNormalMaskBound] using
...
theorem tenMillionHighTailSquarePoint_has_bounded_quotient
    {N pivot point : Nat} (row : TenMillionKernelRootRow)
    (hpivot : pivot ≤ N)
    (hpoint :
  ...
  have hsplit : 0 < row.split := by
    cases row <;> norm_num [TenMillionKernelRootRow.split]
  ...
  have hNsq' : N ^ 2 + 1 ≤ row.bound * p ^ 2 := by
    dsimp [TenMillionKernelRootRow.bound]
    nlinarith
...
#print axioms mixedSupport_length_le_six_of_prod_lt_twentyMillion
#print axioms actualTenMillionPivotSupport_length_le_six
#print axioms actualTenMillionSupport_dropLast_lt_normalMaskBound
#print axioms tenMillionHighTailSquarePoint_has_bounded_quotient
