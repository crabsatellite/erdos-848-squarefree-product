import Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms.OrderBlock0285

namespace Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms

set_option maxRecDepth 1000000
set_option maxHeartbeats 0

theorem evenOneMarker2_25Ordered : evenOneMarker2_25.AdjacentOrdered := by
  apply IndexedMarkerData.adjacentOrdered_of_range (comparisonCount := 2853) (by decide)
  exact orderBlock0285

end Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms
