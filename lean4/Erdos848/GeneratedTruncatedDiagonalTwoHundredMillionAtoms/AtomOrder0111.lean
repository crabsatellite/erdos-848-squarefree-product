import Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms.OrderBlock0255

namespace Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms

set_option maxRecDepth 1000000
set_option maxHeartbeats 0

theorem evenOneMarker1_44Ordered : evenOneMarker1_44.AdjacentOrdered := by
  apply IndexedMarkerData.adjacentOrdered_of_range (comparisonCount := 2859) (by decide)
  exact orderBlock0255

end Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms
