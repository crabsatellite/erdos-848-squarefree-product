import Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms.OrderBlock0247

namespace Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms

set_option maxRecDepth 1000000
set_option maxHeartbeats 0

theorem evenOneMarker1_36Ordered : evenOneMarker1_36.AdjacentOrdered := by
  apply IndexedMarkerData.adjacentOrdered_of_range (comparisonCount := 2836) (by decide)
  exact orderBlock0247

end Erdos848.GeneratedTruncatedDiagonalTwoHundredMillionAtoms
