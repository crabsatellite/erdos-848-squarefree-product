...
import Erdos848.TailFiveMillionTwoCellPeriodicProducer
import Erdos848.GeneratedTailDiagonalCoverage.TailS2EnvelopeAggregate

namespace Erdos848

/-! Direct R263 terminal for the one-odd, two-active-cell row. -/

set_option maxHeartbeats 0
set_option maxRecDepth 1000000

def globalMixedOneOddTwoCellDiagonalEnvelope : Rat :=
  1413097554367904 / 10^18

def globalMixedOneOddTwoCellFiniteEnvelope : Rat :=
  2 * (40 / 147 : Rat) / 25 +
    2 * ((40 / 147 : Rat) + 17120 / 49) / 5_000_000

def globalMixedOneOddTwoCellTailEnvelope : Rat :=
  (2 / 3 : Rat) * fiveMillionOddRoot7Envelope +
    (2 / 3 : Rat) * 6 * fiveMillionSquareTail7Envelope / 25

def globalMixedOneOddTwoCellBudget : Rat :=
  globalMixedOneOddTwoCellDiagonalEnvelope +
    globalMixedOneOddTwoCellFiniteEnvelope +
      globalMixedOneOddTwoCellTailEnvelope + 128 / 5_000_000

theorem globalMixedOneOddTwoCellBudget_le_branchCeiling :
    globalMixedOneOddTwoCellBudget <=
      fiveMillionR263BranchCeiling .oddOneTwoCells := by
  norm_num [globalMixedOneOddTwoCellBudget,
    globalMixedOneOddTwoCellDiagonalEnvelope,
    globalMixedOneOddTwoCellFiniteEnvelope,
    globalMixedOneOddTwoCellTailEnvelope,
    fiveMillionR263BranchCeiling, fiveMillionOddRoot7Envelope,
    fiveMillionSquareTail7Envelope]

theorem globalMixedOneOddTwoCellStructuredDiagonal_kernel_close
    {N : Nat} {B charged : Finset Nat}
    (hLower : 5_000_000 <= N) (hUpper : N < 10_000_000)
    (hBout : Erdos848OutsideSet N B)
    (hBprop : NonSquarefreeProductProp B)
    (parity : Bool) (cells : Finset (Fin 9))
    (hcellCard : cells.card = 2)
    ...
    ((fiveMillionStructuredResidual N B charged).card : Rat) / N <=
      globalMixedOneOddTwoCellDiagonalEnvelope := by
  ...
  calc
    ...
    _ <= globalMixedOneOddTwoCellDiagonalEnvelope := by
      simpa [globalMixedOneOddTwoCellDiagonalEnvelope] using
        GeneratedTailDiagonalCoverage.fiveMillionOneOddS2Diagonal_kernel_close
          N hLower hUpper parity cells (by omega)

noncomputable def globalMixedOneOddTwoCellR263Terminal_of_generated
    {N : Nat} {B charged : Finset Nat}
    (hLower : 5_000_000 <= N) (hUpper : N < 10_000_000)
    (hBout : Erdos848OutsideSet N B)
    (hBprop : NonSquarefreeProductProp B)
  ...
  have hfiniteOriginal :=
    fiveMillionTwoCellBasePeriodicThreshold_ratio_le hLower
      (pattern.toPeriodicCertificate hBout)
  have hfinite :
      fiveMillionR263BaseFiniteThresholdPayment N B pivots 7 4 / N <=
        globalMixedOneOddTwoCellFiniteEnvelope := by
  ...
  have htail : hallBaseTailSquarePayment N B pivots 7 4 / N <=
      globalMixedOneOddTwoCellTailEnvelope := by
    simpa [globalMixedOneOddTwoCellTailEnvelope] using
      hallBaseTailSquarePayment_sixFour_ratio_le tailCertificate
  have hstructuredDiagonal :=
    globalMixedOneOddTwoCellStructuredDiagonal_kernel_close
  ...
  have hresidualN : ((hallResidual N B).card : Rat) / N <=
      globalMixedOneOddTwoCellDiagonalEnvelope + 128 / N := by
    calc
      ...
      _ <= globalMixedOneOddTwoCellDiagonalEnvelope + 128 / N :=
  ...
  have hresidual : ((hallResidual N B).card : Rat) / N <=
      globalMixedOneOddTwoCellDiagonalEnvelope + 128 / 5_000_000 := by
    linarith
  refine
    { branch := .oddOneTwoCells
      pivots := pivots
      cutoff := 7
      threshold := 4
      ...
      thresholdCard := by
        ...
        simpa [pivots] using (show 4 <= pattern.pivots.card by omega)
      pivotsCompletion := hpivotsCompletion
      residualEnvelope :=
        globalMixedOneOddTwoCellDiagonalEnvelope + 128 / 5_000_000
      finiteEnvelope := globalMixedOneOddTwoCellFiniteEnvelope
      tailEnvelope := globalMixedOneOddTwoCellTailEnvelope
  ...
  calc
    globalMixedOneOddTwoCellDiagonalEnvelope + 128 / 5_000_000 +
          globalMixedOneOddTwoCellFiniteEnvelope +
        globalMixedOneOddTwoCellTailEnvelope =
        globalMixedOneOddTwoCellBudget := by
          unfold globalMixedOneOddTwoCellBudget
          ring
    _ <= fiveMillionR263BranchCeiling .oddOneTwoCells :=
      globalMixedOneOddTwoCellBudget_le_branchCeiling

#print axioms globalMixedOneOddTwoCellBudget_le_branchCeiling
