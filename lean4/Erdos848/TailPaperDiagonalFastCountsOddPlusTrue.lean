...
theorem TruncatedDiagonalAtomCounts.fastSum_oddPlusTrue_eq_sum
    (counts : TruncatedDiagonalAtomCounts) (cell : Fin 9) :
    counts.fastSum (.oddPlusCell true cell) =
      counts.sum (.oddPlusCell true cell) := by
  apply counts.fastSum_eq_sum_of_code
  let g : TruncatedDiagonalAtomCode → Nat := fun code =>
    if (PaperDiagonalSelection.oddPlusCell true cell).AcceptsAtom
        (truncatedDiagonalAtomEquiv.symm code)
      then counts.count (truncatedDiagonalAtomEquiv.symm code) else 0
  change counts.fastSum (.oddPlusCell true cell) = ∑ code, g code
...
#print axioms TruncatedDiagonalAtomCounts.fastSum_oddPlusTrue_eq_sum
